...
// filter builds via command line arg过滤器构建通过命令行参数
if (process.argv[2]) {
  ...
  // filter out weex builds by default默认情况下过滤掉weex构建
  builds = builds.filter(b => {
...
```js
const builds = {
  // Runtime only (CommonJS). Used by bundlers e.g. Webpack & Browserify运行时只(CommonJS)。由bundlers使用，如Webpack和Browserify
  'web-runtime-cjs': {
  ...
  // Runtime+compiler CommonJS build (CommonJS)运行时+编译器 CommonJS(CommonJS)构建
  'web-full-cjs': {
...
//例如Rollup和Webpack 2
  ...
  // Runtime+compiler CommonJS build (ES Modules)运行时+编译器 CommonJS(ES Modules)构建
  'web-full-esm': {
    entry: resolve('web/entry-runtime-with-compiler.js'),
  ...
  // runtime-only build (Browser)仅运行时构建(Browser)
  'web-runtime-dev': {
    entry: resolve('web/entry-runtime.js'),
  ...
  // runtime-only production build (Browser)仅运行时 生产环境构建(Browser)
  'web-runtime-prod': {
    entry: resolve('web/entry-runtime.js'),
  ...
  // Runtime+compiler development build (Browser)运行时+编译器 生产环境构建 (Browser)
  'web-full-dev': {
    entry: resolve('web/entry-runtime-with-compiler.js'),
  ...
  // Runtime+compiler production build  (Browser)运行时+编译器 生产环境构建(Browser)
  'web-full-prod': {
    entry: resolve('web/entry-runtime-with-compiler.js'),
